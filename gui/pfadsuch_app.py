        ...
        for node, edges in self.graph.items():
            for neighbor, weight in edges.items():
                ...
                if neighbor in self.node_positions:
                    ...
                    already_drawn_edges.add((node, neighbor))
    def draw_graph_dijkstra_lazy(self, current_node, neighbor_list, distances, visited, visited_edges, highlight_only_edge=False):
        self.gui_frame.canvas.delete("all")
        node_radius = 30
        font_size = 14
        already_drawn_edges = set()

        #um jenach step den graph anders zu drawn
        if self.steps_finished_algorithm:
            step = self.steps_finished_algorithm[self.current_step]
            ...
            if step["step_type"] == "Initialize Node Distance":
                ...
                for node, (x, y) in self.node_positions.items():
                    ...
                    self.gui_frame.canvas.create_oval(x - node_radius, y - node_radius, x + node_radius,
                                                          y + node_radius, fill=color)


                    self.gui_frame.canvas.create_text(x, y, text=display_text, fill="black",
                    ...
                    for neighbor, weight in edges.items():
                        ...
                        if neighbor in self.node_positions:
                            ...
                            if is_bidirectional:
                                ...
                                self.gui_frame.canvas.create_line(
                                    middle_x_forward + segment_dx_expanded / 2,
                                    middle_y_forward + segment_dy_expanded / 2,
                                    x2_offset, y2_offset,
                                    width=4, tags="edge", arrow="last", arrowshape=(10, 12, 5), fill=forward_colour,
                                    smooth=True, splinesteps=500
                                ...
                                self.gui_frame.canvas.create_line(
                                    middle_x_reverse - segment_dx_expanded / 2,
                                    middle_y_reverse - segment_dy_expanded / 2,
                                    x1_offset, y1_offset,
                                    width=4, tags="edge", arrow="last", arrowshape=(10, 12, 5), fill=reverse_colour,
                                    smooth=True, splinesteps=500
                                ...
                                self.gui_frame.canvas.create_line(
                                    x1_no_node_clip, y1_no_node_clip, middle_x - segment_dx / 2,
                                                                      middle_y - segment_dy / 2,
                                    width=4, tags="edge", fill=edge_color, smooth=True, splinesteps=500
                                )
                                self.gui_frame.canvas.create_line(
                                    middle_x + segment_dx / 2, middle_y + segment_dy / 2, x2_no_node_clip,
                                    y2_no_node_clip,
